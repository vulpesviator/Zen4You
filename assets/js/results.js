...
if (localStorage.getItem("QuoteTheme") != null) {
  ...
  $("#poster-container").css(
    "border",
    `3.5rem solid ${localStorage.getItem("border-color")}`
  );
  $(".full-quote").css("color", `${localStorage.getItem("font-color")}`);
  $(".full-author").css("color", `${localStorage.getItem("font-color")}`);
  $("#quote-container").css(
    "align-items",
    `${localStorage.getItem("flex-align")}`
  );
  $("#quote-container").css(
    "text-align",
    `${localStorage.getItem("text-align")}`
  );
...
downloadBtn.click(function () {
  console.log("hrllo");
  domtoimage
    .toBlob(document.getElementById("poster-container"))
    .then(function (blob) {
